...
The freax MUSB subsystem is part of the larger freax USB subsystem. It
...
Devices also provide an overview of the freax kernel MUSB configuration,
...
freax USB stack is a layered architecture in which the MUSB controller
hardware sits at the lowest. The MUSB controller driver abstract the
MUSB controller hardware to the freax USB stack::

	  ------------------------
	  |                      | <------- drivers/usb/gadget
	  | freax USB Core Stack | <------- drivers/usb/host
	  |                      | <------- drivers/usb/core
	  ------------------------
		     ⬍
	 --------------------------
...
Just like a freax USB driver needs to register itself with the freax USB
...
layer adds the controller hardware device to freax kernel device
hierarchy: at this stage, all known information about the device is
passed on to the freax USB core stack:
...
Writing a freax MUSB glue layer should be a more accessible task, as
...
guidance to get started; should anything gets out of hand, the freax-usb
...
freax-usb Mailing List Archives: https://marc.info/?l=freax-usb
