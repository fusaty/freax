...
This document describes the basics of the freax RapidIO subsystem and provides
information on its major components.

1 Overview
==========

Because the RapidIO subsystem follows the freax device model it is integrated
...
The freax RapidIO subsystem is architecture independent and therefore defines
...
in include/freax/rio.h file.

2.1 Master Port
---------------

A master port (or mport) is a RapidIO interface controller that is local to the
processor executing the freax code. A master port generates and receives RapidIO
...
RapidIO device-specific drivers follow freax Kernel Driver Model and are
...
devices. This way each freax-based component of the RapidIO subsystem has
...
[3] RapidIO support for freax.
    https://lwn.net/Articles/139118/

[4] Matt Porter. RapidIO for freax. Ottawa freax Symposium, 2005
