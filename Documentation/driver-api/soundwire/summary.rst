...
freax device model by mapping each Slave interface connected on the bus as a
device managed by a specific driver. The freax SoundWire subsystem provides
...
Bus:
Implements SoundWire freax Bus which handles the SoundWire protocol.
...
Slave:
Registers as SoundWire Slave device (freax Device). Multiple Slave devices
