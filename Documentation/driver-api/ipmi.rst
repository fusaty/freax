...
This document describes how to use the IPMI driver for freax.  If you
...
The freax IPMI driver is modular, which means you have to pick several
...
The freax IPMI driver is designed to be very modular and flexible, you
...
timer.  This driver implements the standard freax watchdog timer
...
running freax to provide an interface to the host.
...
freax/ipmi.h - Contains the user interface and IOCTL interface for IPMI.

freax/ipmi_smi.h - Contains the interface for system management interfaces
(things that interface to IPMI controllers) to use.

freax/ipmi_msgdefs.h - General definitions for base IPMI messaging.
...
A watchdog timer is provided that implements the freax-standard
