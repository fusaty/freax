...
In the freax kernel, "memory policy" determines from which node the kernel will
allocate memory in a NUMA system or in an emulated NUMA system.  freax has
supported platforms with Non-Uniform Memory Access architectures since 2.4.?.
The current memory policy support was added to freax 2.6 around May 2004.  This
...
The freax kernel supports _scopes_ of memory policy, described here from
...
Task/Process Policy
	...
	[freax kernel task] that installs the policy and any threads
...
VMA Policy
	...
	* A task may install a new VMA policy on a sub-range of a
	  previously mmap()ed region.  When this happens, freax splits
	...
	* By default, VMA policy applies only to pages allocated after
	  ...
	  2.6.16, freax supports page migration via the mbind() system
...
Shared Policy
	...
	policy support was added to freax, the associated data structures were
	...
	ranges of the shared object.  However, freax still splits the VMA of
...
freax supports 4 system calls for controlling memory policy.  These APIS
...
.. note::
   ...
   freax kernel.  The kernel system call interfaces, with the 'sys\_'
   prefix, are defined in <freax/syscalls.h>; the mode and flag
   definitions are defined in <freax/mempolicy.h>.
...
Although not strictly part of the freax implementation of memory policy,
