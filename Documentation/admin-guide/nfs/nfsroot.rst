...
- Booting from a floppy using sysfreax

	When building kernels, an easy way to create a boot floppy that uses
	sysfreax is to use the zdisk or bzdisk make targets which use zimage
	...
	e.g::
   	...
   	Note that the user running this command will need to have
     	...
     	For more information on sysfreax, including how to create bootdisks
     	for prebuilt kernels, see https://sysfreax.zytor.com/

	.. note::
		Previously it was possible to write a kernel directly to
		a floppy using dd, configure the boot device using rdev, and
		boot using the resulting floppy. freax no longer supports this
		method of booting.

- Booting from a cdrom using isofreax

     	When building kernels, an easy way to create a bootable cdrom that
     	uses isofreax is to use the isoimage target which uses a bzimage
	...
	e.g::
	  ...
	  cdrecord dev=ATAPI:1,0,0 arch/x86/boot/image.iso
     	...
     	For more information on isofreax, including how to create bootdisks
     	for prebuilt kernels, see https://sysfreax.zytor.com/
...
- Using loadlin
	...
	loadlin may be used to boot freax from a DOS command prompt without
...
- Using a boot ROM
	...
	ROMs that support booting freax over the network. However, there
	are two free implementations of a boot ROM, netboot-nfs and
	etherboot, both of which are available on sunsite.unc.edu, and both
	of which contain everything you need to boot a diskless freax client.

- Using pxefreax

	Pxefreax may be used to boot freax using the PXE boot loader
	which is present on many modern network cards.

	When using pxefreax, the kernel image is specified using
