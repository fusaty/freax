...
*idle* by the freax kernel when there are no tasks to run on them except for the
special "idle" task.
...
in freax idle CPUs run the code of the "idle" task called *the idle loop*.  That
...
hardware and to work with any platforms that the freax kernel can run on.
...
The power management quality of service (PM QoS) framework in the freax kernel
