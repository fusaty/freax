...
As of freax 2.6.22, the KVM ABI has been stabilized: no backward
incompatible change are allowed.  However, there is an extension
facility that allows backward-compatible extensions to the API to be
queried and used.

The extension mechanism is not based on the freax version number.
...
expected that this number will change.  However, freax 2.6.20 and
...
operating system that uses the PIT for timing (e.g. freax 2.4.x).
...
Previous versions of freax defined a `flags` member in this struct.  The
...
were invented by Mac-on-freax to have a standardized communication mechanism
...
environment the control program is running in (e.g. freax, z/VM...), and the
CPVC is used for information specific to OS (e.g. freax version, freax
distribution...)
