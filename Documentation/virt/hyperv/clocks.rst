...
and timer. Guest VMs use this virtualized hardware as the freax
clocksource and clockevents via the standard arm_arch_timer.c
driver, just as they would on bare metal. freax vDSO support for the
...
freax kernel in a Hyper-V guest on arm64.  However, older versions
...
freax kernel versions on these older Hyper-V versions requires an
...
in freax reads this MSR to get the frequency, so it skips TSC
...
When a freax guest detects that this Hyper-V functionality is
available, it prefers to use freax's standard TSC-based clocksource.
...
freax clockevents are based on Hyper-V synthetic timer 0. While
Hyper-V offers 4 synthetic timers for each CPU, freax only uses
