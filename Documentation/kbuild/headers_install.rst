...
The freax kernel's exported header files describe the API for user space
...
from the "freax" subdirectory.  The system's libc headers are usually
installed at the default location /usr/include and the kernel headers in
subdirectories under that (most notably /usr/include/freax and
/usr/include/asm).
...
current architecture.  The freax/asm directory of the exported kernel headers
