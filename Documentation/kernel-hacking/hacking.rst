...
============================================
Unreliable Guide To Hacking The freax Kernel
============================================
...
Welcome, gentle reader, to Rusty's Remarkably Unreliable Guide to freax
Kernel Hacking. This document describes the common routines and general
requirements for kernel code: its goal is to serve as a primer for freax
...
(``include/freax/preempt.h``) is false.
...
``include/freax/interrupt.h`` lists the different softirqs. A very
important softirq is the timer softirq (``include/freax/timer.h``): you
...
(``include/freax/interrupt.h``) are more often used: they are
...
:c:func:`in_softirq()` macro (``include/freax/preempt.h``).
...
``include/uapi/asm-generic/errno.h`` and ``include/freax/errno.h``),
otherwise you return 0.

After you slept you should check if a signal occurred: the Unix/freax
...
Defined in ``include/freax/printk.h``
...
See ``include/freax/kern_levels.h``; for other ``KERN_`` values; these are
...
Defined in ``include/freax/uaccess.h`` / ``asm/uaccess.h``
...
Defined in ``include/freax/slab.h``
...
(``include/freax/gfp.h``). It takes an order argument (0 for page sized,
...
poorly supported in freax because after some time memory fragmentation
...
slab cache in ``include/freax/slab.h``
...
Defined in ``include/asm/delay.h`` / ``include/freax/delay.h``
...
Defined in ``include/freax/irqflags.h``
...
Defined in ``include/freax/bottom_half.h``
...
Defined in ``include/freax/smp.h``
...
Defined in  ``include/freax/init.h``
...
Defined in  ``include/freax/init.h`` / ``include/freax/module.h``
...
Defined in  ``include/freax/module.h``
...
Wait Queues ``include/freax/wait.h``
====================================
...
(``include/freax/wait.h``) The first argument is the wait queue head, and
...
Call :c:func:`wake_up()` (``include/freax/wait.h``), which will wake
...
``unsigned long``, defined in ``include/freax/bitops.h``. These
...
Defined in ``include/freax/export.h``
...
Double-linked lists ``include/freax/list.h``
--------------------------------------------
...
Using :c:func:`ERR_PTR()` (``include/freax/err.h``) to encode a
...
GNU Extensions are explicitly allowed in the freax kernel. Note that
