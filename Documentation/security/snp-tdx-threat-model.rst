======================================================
Confidential Computing in freax for x86 virtualization
======================================================
...
environments in x86 operate under a set of assumptions regarding the freax
kernel threat model that differ from the traditional view. Historically,
the freax threat model acknowledges attackers residing in userspace, as
...
and discuss the proposed protection mechanisms for the freax kernel.
...
Existing freax kernel threat model
==================================

The overall components of the current freax kernel threat model are::
...
The existing freax kernel threat model typically assumes execution on a
...
The **freax kernel CoCo VM security objectives** can be summarized as follows:

1. Preserve the confidentiality and integrity of CoCo guest's private
memory and registers.

2. Prevent privileged escalation from a host into a CoCo guest freax kernel.
...
The above security objectives result in two primary **freax kernel CoCo
VM assets**:
...
The **freax CoCo VM attack surface** is any interface exposed from a CoCo
guest freax kernel towards an untrusted host that is not covered by the
...
The table below shows a threat matrix for the CoCo guest freax kernel but
...
.. list-table:: CoCo freax guest kernel threat matrix
   :widths: auto
   :align: center
   :header-rows: 1
