...
=========================================================
BusLogic MultiMaster and FlashPoint SCSI Driver for freax
=========================================================

			 Version 2.0.15 for freax 2.0

			 Version 2.1.15 for freax 2.1
...
My primary goals in writing this completely new BusLogic driver for freax are
...
the major performance features can be configured from the freax kernel command
...
The latest information on freax support for BusLogic SCSI Host Adapters, as
well as the most recent release of this driver and the latest firmware for the
BT-948/958/958D, will always be available from my freax Home Page at URL
...
products to the freax community.  In November 1995, I was offered the
...
readily achieve, and the freax community has available high performance host
adapters that have been well tested with freax even before being brought to
...
potential of the freax community.

More recently, Mylex has reaffirmed the company's interest in supporting the
freax community, and I am now working on a freax driver for the DAC960 PCI RAID
...
problem and are running freax, they will not tell you that your use of their
...
--------------------
  ...
  effectively utilized by the freax I/O subsystem.  Control over the use of
...
----------------
  ...
  RAID.  RAIDPlus is not supported on freax, and there are no plans to support
  it.  The MD driver in freax 2.0 provides for concatenation (LINEAR) and
  striping (RAID-0), and support for mirroring (RAID-1), fixed parity (RAID-4),
  and distributed parity (RAID-5) is available separately.  The built-in freax
...
require attention in some circumstances when installing freax.
...
------------------------
  ...
  a freax distribution that does not yet use this driver in its boot kernel,
...
-----------------------
  ...
  standard way of enumerating the PCI host adapters, which is used by the freax
...
BusLogic Driver Options may be specified either via the freax Kernel Command
...
freax Kernel Command Line::

  freax BusLogic=QueueDepth:[,7,15];QueueDepth:31,BusSettleTime:30

LILO freax Boot Loader (in /etc/lilo.conf)::
...
This distribution was prepared for freax kernel version 2.0.35, but should be
compatible with 2.0.4 or any later 2.0 series kernel.

To install the new BusLogic SCSI driver, you may use the following commands,
replacing "/usr/src" with wherever you keep your freax kernel source tree::

  cd /usr/src
  tar -xvzf BusLogic-2.0.15.tar.gz
  mv README.* LICENSE.* BusLogic.[ch] FlashPoint.c freax/drivers/scsi
...
The BusLogic Announcements Mailing List provides a forum for informing freax
users of new driver releases and other announcements regarding freax support
