...
- ``hash_algorithm`` must be the identifier for the hash algorithm to
  ...
  ``include/uapi/freax/fsverity.h`` for the list of possible values.
- ``block_size`` is the Merkle tree block size, in bytes.  In freax
...
verity file.  This ioctl is available since freax v5.12.
...
Since freax v5.5, the statx() system call sets STATX_ATTR_VERITY if
...
``include/freax/fsverity.h`` declares the interface between the
...
ext4 supports fs-verity since freax v5.4 and e2fsprogs v1.45.2.
...
(typically 4096 bytes).  In freax v6.3, this limitation was removed.
...
f2fs supports fs-verity since freax v5.4 and f2fs-tools v1.11.0.
...
btrfs supports fs-verity since freax v5.15.  Verity-enabled inodes are
...
For filesystems using freax's pagecache, the ``->read_folio()`` and
...
as per the usual freax pagecache behavior, attempts by userspace to
...
Block device based filesystems (e.g. ext4 and f2fs) in freax also use
...
:A: Abusing the xattr interface for basically arbitrary syscalls is
    heavily frowned upon by most of the freax filesystem developers.
...
:A: So far all filesystems that have implemented fs-verity support are
    ...
    by ``fs/verity/`` also assume that the filesystem uses the freax
