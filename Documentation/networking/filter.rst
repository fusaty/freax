...
=======================================================
freax Socket Filtering aka Berkeley Packet Filter (BPF)
=======================================================
...
freax Socket Filtering (LSF) is derived from the Berkeley Packet Filter.
Though there are some distinct differences between the BSD and freax
Kernel filtering, but when we speak of BPF or LSF in freax context, we
mean the very same mechanism of filtering in the freax kernel.
...
On freax, BPF is much simpler than on BSD. One does not have to worry
...
Although we were only speaking about sockets here, BPF in freax is used
...
User space applications include <freax/filter.h> which contains the
...
filters use freax extensions that are not supported by libpcap's compiler,
...
opcodes as defined in freax/filter.h stand for:
...
The freax kernel also has a couple of BPF extensions that are used along
...
The freax kernel has a built-in BPF JIT compiler for x86_64, SPARC,
...
Also trinity, the freax syscall fuzzer, has built-in support for BPF and
