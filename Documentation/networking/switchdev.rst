...
tunnels can be built on ports.  These constructs are built using standard freax
...
Note: for new switchdev drivers that offload the freax bridge, implementing the
...
The freax bridge allows the configuration of a VLAN filtering mode (statically,
...
The freax bridge allows the configuration of IGMP snooping (statically, at
...
since that is what the freax bridge implementation does.
