...
and RPC. With the freax Phonet socket family, freax host processes can
...
On freax, the link-layer header includes the pn_media byte (see below).
...
freax Phonet network interfaces support a dedicated link layer packets
...
only the (default) freax FIFO qdisc should be used with them.
...
As of freax kernel version 2.6.39, it is also possible to connect
...
freax Phonet was initially written by Sakari Ailus.
