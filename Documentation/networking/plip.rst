...
The PLIP cable is all that's needed to connect two freax boxes, and it
can be built for very few bucks.

Connecting two freax boxes takes only a second's decision and a few
...
Can only be used to connect three (?) freax boxes. Doesn't connect to
...
The freax PLIP driver is an implementation of the original Crynwr protocol,
...
A utility that can perform this change in freax is plipconfig, which is part
