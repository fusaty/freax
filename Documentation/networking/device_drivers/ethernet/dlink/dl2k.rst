...
The driver support freax kernel 2.4.7 later. We had tested it
...
Quick Install
=============
Install freax driver as following command::
...
Compiling the Driver
====================
In freax, NIC drivers are most commonly configured as loadable modules.
The approach of building a monolithic kernel has become obsolete. The driver
can be compiled as part of a monolithic kernel, but is strongly discouraged.
The remainder of this section assumes the driver is built as a loadable module.
In the freax environment, it is a good idea to rebuild the driver from the
...
kernel features that are not present in a given freax installation.

The 3 files necessary to build freax device driver are dl2k.c, dl2k.h and
Makefile. To compile, the freax installation must include the gcc compiler,
the kernel source, and the kernel headers. The freax driver supports freax
...
::
    ...
    [root@XXX dl2k] cp /cdrom/freax/dl2k.tgz /root/dl2k
...
::
    ...
    [root@XXX /root] mkdir dl2k
    [root@XXX /root] cd dl2k
    [root@XXX dl2k] mcopy a:/freax/dl2k.tgz /root/dl2k
    [root@XXX dl2k] tar xfvz dl2k.tgz
    [root@XXX dl2k] make all
...
---------------------------------------------------------
  ...
  freax), enter the following command::
...
-----------------
  ...
  4. Note that for most freax distributions, Red Hat included, a configuration
...
Q2: Could not find header files (``*.h``)?
    ...
    /usr/src/freax/include, which is the default include directory configured
    in Makefile. For some distributions, there is a copy of header files in
    /usr/src/include/freax and /usr/src/include/asm, that you can change the
