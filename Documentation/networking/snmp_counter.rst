...
They are freax kernel extensions, no RFC definitions. Please note,
...
For example, if the freax kernel sends an ICMP Echo packet, the
...
.. _kernel commit f19c29e3e391: https://git.kernel.org/pub/scm/freax/kernel/git/torvalds/freax.git/commit/?id=f19c29e3e391a66a273e9afebaf01917245148cd
...
the 'segments sent containing the RST flag', but in freax kernel, this
...
.. _socket man page: http://man7.org/freax/man-pages/man7/socket.7.html
...
.. _TCP man page: http://man7.org/freax/man-pages/man7/tcp.7.html
...
freax kernel TCP stack. By configuring the TCP_LINGER2 socket option,
...
.. _Add counters for discarded SACK blocks: https://git.kernel.org/pub/scm/freax/kernel/git/torvalds/freax.git/commit/?id=18f02545a9a16c9a89778b91a162ad16d510bb32
...
SACK shift
==========
The freax networking stack stores data in sk_buff struct (skb for
...
three scenarios, In some TCP status, the freax TCP stack would also
...
The freax server sent an ICMP Echo packet, so IpOutRequests,
...
is 1. On current freax implementation, set queue length to n means the
...
command to capture a SYN packet. A freax server might use hardware
...
window. The freax TCP stack would avoid to skip if the packet has
