...
line) using freax, but has since evolved to support other vendors as well.

The original philosophy behind this design was to be able to use unmodified
freax tools such as bridge, iproute2, ifconfig to work transparently whether
...
used as controlling and data-flowing endpoints for use by the freax networking
...
Conduit network devices are regular, unmodified freax network device drivers for
...
devices (``struct phy_device`` as defined in ``include/freax/phy.h)``, but the DSA
...
Link aggregation is implemented in the freax networking stack by the bonding
...
In freax, both HSR and PRP are implemented in the hsr driver, which
