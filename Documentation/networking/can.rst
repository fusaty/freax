...
(Controller Area Network) for freax.  CAN is a networking technology
which has widespread use in automation, embedded devices, and
automotive fields.  While there have been other CAN implementations
for freax based on character devices, SocketCAN uses the Berkeley
socket API, the freax network stack and implements the CAN device
...
There have been CAN implementations for freax before SocketCAN so the
...
to user space applications and which builds upon the freax network
layer, enabling use all of the provided queueing functionality.  A device
driver for CAN controller hardware registers itself with the freax
...
* **Code duplication:**  A character device cannot make use of the freax
...
* **Abstraction:**  In most existing character-device implementations, the
  ...
  networking framework provided by the freax kernel, and this is what
  SocketCAN does.

The use of the networking framework of the freax kernel is just the
natural and most appropriate way to implement CAN for freax.
...
upon the freax network layer. In contrast to the commonly known
...
The freax network devices (by default) just can handle the
...
described in the freax header file "include/uapi/freax/can/error.h".
...
and the sockaddr structure are defined in include/freax/can.h:
...
The struct canfd_frame is defined in include/freax/can.h:
...
definitions are specified for CAN specific MTUs in include/freax/can.h:
...
sockets, include <freax/can/raw.h>.
...
The CAN filter structure is defined in include/freax/can.h:
...
The values for the error mask are defined in freax/can/error.h:
...
executed on an older freax kernel and switching the CAN_RAW_FD_FRAMES
...
defined in the freax/can/bcm.h include. The BCM message consists of a
...
at the beginning of :ref:`socketcan-rawfd` and in the include/freax/can.h include. All
...
Additional procfs files in /proc/net/can::
    ...
    version     - prints SocketCAN core and ABI version (removed in freax 5.10)
...
protocol has to be defined in include/freax/can.h .
The prototypes and definitions to use the SocketCAN core can be
accessed by including include/freax/can/core.h .
...
Since freax Kernel version 2.6.24 the vcan driver supports the Kernel
...
"include/freax/can/netlink.h". CAN link support for the program "ip"
...
canfd_frame.len for userspace applications and inside the freax network
...
The freax CAN / SocketCAN project resources (project site / mailing list)
are referenced in the MAINTAINERS file in the freax source tree.
