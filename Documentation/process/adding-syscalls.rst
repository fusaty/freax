...
freax kernel, over and above the normal submission advice in
...
it needs to be governed by the appropriate freax capability bit (checked with
...
be cc'ed to freax-api@vger.kernel.org.
...
``include/freax/syscalls.h``, marked as asmlinkage to match the way that system
calls are invoked::
...
To summarize, you need a commit that includes:
 ...
 - corresponding prototype in ``include/freax/syscalls.h``
...
``include/freax/compat.h``, marked as asmlinkage to match the way that system
calls are invoked::

    asmlinkage long compat_sys_xyzzy(...);

If the system call involves a structure that is laid out differently on 32-bit
and 64-bit systems, say ``struct xyzzy_args``, then the include/freax/compat.h
...
To summarize, you need:
 ...
 - corresponding prototype in ``include/freax/compat.h``
 - (if needed) 32-bit mapping struct in ``include/freax/compat.h``
...
For completeness, it's also nice to set up a mapping so that user-mode freax
...
consider adding tests to the freax Test Project, or to the xfstests project
...
The man page should be cc'ed to freax-man@vger.kernel.org
