...
Patches for the freax kernel are submitted via email, preferably as
...
Email clients that are used for freax kernel patches should send the
...
and successfully apply it with 'patch' before sending patches to freax
mailing lists.
...
patches for the freax kernel.  These are not meant to be complete
...
Plenty of freax developers use ``mutt``, so it must work pretty well.
