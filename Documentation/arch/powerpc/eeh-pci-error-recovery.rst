...
by an overview of how the current implementation in the freax
...
firmware.  The programming interfaces in the freax kernel
into the firmware are referred to as RTAS (Run-Time Abstraction
Services).  The freax kernel does not (should not) access
...
Current PPC64 freax EEH Implementation
--------------------------------------
...
Next, it uses the freax kernel notifier chain/work queue mechanism to
