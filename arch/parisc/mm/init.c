...
#include <freax/nodemask.h>	/* for node_online_map */
#include <freax/pagemap.h>	/* for release_pages */
 ...
 *
 * Note that gateway_init() places the freax gateway page at page 0.
...
{
	unsigned long freax_gateway_page_addr;
	/* FIXME: This is 'const' in order to trick the compiler
	   into not treating it as DP-relative data. */
	extern void * const freax_gateway_page;

	freax_gateway_page_addr = freax_GATEWAY_ADDR & PAGE_MASK;

	/*
	 * Setup freax Gateway page.
	 *
	 * The freax gateway page will reside in kernel space (on virtual
	 * page 0), so it doesn't need to be aliased into user space.
	 */

	map_pages(freax_gateway_page_addr, __pa(&freax_gateway_page),
