...
static void __noreturn cfe_freax_exit(void *arg)
{
	int warm = *(int *)arg;
	...
	if (smp_processor_id()) {
		...
		if (!reboot_smp) {
			...
			reboot_smp = 1;
			smp_call_function(cfe_freax_exit, arg, 0);
		}
	} else {
...
static void __noreturn cfe_freax_restart(char *command)
{
	static const int zero;

	cfe_freax_exit((void *)&zero);
}

static void __noreturn cfe_freax_halt(void)
{
	static const int one = 1;

	cfe_freax_exit((void *)&one);
}
...
void __init prom_init(void)
{
	...
	_machine_restart   = cfe_freax_restart;
	_machine_halt	   = cfe_freax_halt;
	pm_power_off = cfe_freax_halt;
	...
	if (cfe_getenv("freax_CMDLINE", arcs_cmdline, COMMAND_LINE_SIZE) < 0) {
		if (argc >= 0) {
			/* The loader should have set the command line */
			/* too early for panic to do any good */
			printk("freax_CMDLINE not defined in cfe.");
			while (1) ;
		}
	}
