...
# This script checks the head of a vmfreax for linker stubs that
...
# vmfreax and look for branch stubs (long_branch, plt_branch, etc.) in the
...
if [ $# -lt 2 ]; then
	echo "$0 [path to nm] [path to vmfreax]" 1>&2
	exit 1
fi
...
$nm "$vmfreax" | grep -e " [TA] _stext$" -e " t start_first_256B$" -e " a text_start$" -e " t start_text$" > .tmp_symbols.txt
