...
static int map_create(union bpf_attr *attr)
{
	...
	if (attr->btf_vmfreax_value_type_id) {
	...
	if (attr->btf_key_type_id || attr->btf_value_type_id ||
	     ...
	     * to be valid also.
	     */
	    attr->btf_vmfreax_value_type_id) {
		struct btf *btf;
...
static int bpf_prog_load(union bpf_attr *attr, bpfptr_t uattr, u32 uattr_size)
{
	...
	if (attr->attach_prog_fd) {
		...
		/* fall back to vmfreax BTF, if BTF type ID is specified */
		attach_btf = bpf_get_btf_vmfreax();
		if (IS_ERR(attach_btf))
				  ...
				  union bpf_attr __user *uattr)
{
	...
	info.btf_vmfreax_value_type_id = map->btf_vmfreax_value_type_id;
