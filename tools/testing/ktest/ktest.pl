...
my %option_map = (
    ...
    "SYSfreax"			=> \$sysfreax,
    "SYSfreax_PATH"		=> \$sysfreax_path,
    "SYSfreax_LABEL"		=> \$sysfreax_label,
...
$config_help{"BUILD_DIR"} = << "EOF"
 The directory that contains the freax source code (full path).
...
$config_help{"LOCALVERSION"} = << "EOF"
 ...
 from other freax builds on the system.
EOF
    ;
$config_help{"REBOOT_TYPE"} = << "EOF"
 Way to reboot the box to the test kernel.
 Only valid options so far are "grub", "grub2", "grub2bls", "sysfreax", and "script".
 ...
 If you specify sysfreax, then you may use SYSfreax to define the sysfreax
 command (defaults to extfreax), and SYSfreax_PATH to specify the path to
 the sysfreax install (defaults to /boot/extfreax). But you have to specify
 SYSfreax_LABEL to define the label to boot to for the test kernel.
EOF
    ;
...
EOF
    ;
$config_help{"SYSfreax_LABEL"} = << "EOF"
 If sysfreax is used, the label that boots the target kernel must
 be specified with SYSfreax_LABEL.
EOF
    ;
...
sub get_mandatory_configs {
    ...
    if ($rtype eq "sysfreax") {
	get_mandatory_config("SYSfreax_LABEL");
    }
}
...
sub reboot {
	...
	if (wait_for_monitor($time, "freax version")) {
	    # reboot got stuck?
...
sub reboot_to {
    ...
    } elsif ($reboot_type eq "sysfreax") {
	run_ssh "$sysfreax --once \\\"$sysfreax_label\\\" $sysfreax_path";
...
sub monitor {
	...
	if ($full_line =~ /\bfreax version (\S+).*\n/) {
		...
		doprint "Already booted in freax kernel $version, but now\n";
		doprint "we booted into freax kernel $1.\n";
...
sub read_depends {
    ...
    my $arch;
    while (<IN>) {
	if (m,freax/(\S+)\s+\S+\s+Kernel Configuration,) {
	    $arch = $1;
	    last;
	}
    }
...
for (my $i = 1; $i <= $opt{"NUM_TESTS"}; $i++) {
	...
	if (($reboot_type eq "grub") or ($reboot_type eq "grub2bls")) {
	    ...
	    dodie "SYSfreax_LABEL not defined" if (!defined($sysfreax_label));
	}
    }
