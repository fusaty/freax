...
{
	/*
	 * Usually vmfreax is an ELF file with type ET_EXEC for most
		 ...
		 enum dso_binary_type type)
{
	...
	if (dso->has_build_id && !symbol_conf.ignore_vmfreax_buildid) {
		  ...
		  struct symsrc *runtime_ss, int kmodule)
{
	...
	if (!syms_ss->symtab) {
		/*
		 * If the vmfreax is stripped, fail so we will fall back
		 * to using kallsyms. The vmfreax runtime symbols aren't
		 * of much use.
		 */
		if (dso->kernel)
			return err;
