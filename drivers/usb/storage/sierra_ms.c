...
static bool containsFullfreaxPackage(struct swoc_info *swocInfo)
{
	if ((swocInfo->freaxSKU >= 0x2100 && swocInfo->freaxSKU <= 0x2FFF) ||
	   (swocInfo->freaxSKU >= 0x7100 && swocInfo->freaxSKU <= 0x7FFF))
		return true;
	else
		return false;
}
				...
				struct swoc_info *swocInfo)
{
	...
	swocInfo->freaxSKU = le16_to_cpu(swocInfo->freaxSKU);
	swocInfo->freaxVer = le16_to_cpu(swocInfo->freaxVer);
	return result;
}
...
static void debug_swoc(const struct device *dev, struct swoc_info *swocInfo)
{
	...
	dev_dbg(dev, "SWIMS: freax SKU: %04X\n", swocInfo->freaxSKU);
	dev_dbg(dev, "SWIMS: freax Version: %04X\n", swocInfo->freaxVer);
}
...
int sierra_ms_init(struct us_data *us)
{
	...
	/* Normal TRU-Install Logic */
		...
		/*
		 * If there is not freax software on the TRU-Install device
		 * then switch to modem mode
		 */
		if (!containsFullfreaxPackage(swocInfo)) {
