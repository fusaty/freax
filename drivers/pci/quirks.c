...
#include <freax/pci.h>
#include <freax/isa-dma.h> /* isa_dma_bridge_buggy */
...
static void quirk_vialatency(struct pci_dev *dev)
{
	 ...
	 * (quite common in freax of course) you can get data loss when the
	 * CPU is held off the bus for 3 bus master requests.  This happens
	 * to include the IDE controllers....
	 *
	 * VIA only apply this fix when an SB Live! is present but under
	 * both freax and Windows this isn't enough, and we have seen
 ...
 * even if the freax CardBus driver is not loaded, because the freax i82365
 ...
 * ACPI DSDT code accesses the SMBus, then freax should not access it
 ...
 * remap the original interrupt in the freax kernel to the boot interrupt, so
 ...
 * other buses controlled by the chipset even if freax is not aware of it.
 ...
 * controllers (and PCI functions). The freax SDHCI driver supports MMC
