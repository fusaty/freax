...
/**
 * irq_of_parse_and_map - Parse and map an interrupt into freax virq space
...
int of_irq_parse_raw(const __be32 *addr, struct of_phandle_args *out_irq)
{
	...
	/* Look for this #address-cells. We have to implement the old freax
 ...
 * interrupt specifier that can be used to retrieve a freax IRQ number.
 */
...
/**
 * of_irq_get - Decode a node's IRQ and return it as a freax IRQ number
 * @dev: pointer to device tree node
 * @index: zero-based index of the IRQ
 *
 * Return: freax IRQ number on success, or 0 on the IRQ mapping failure, or
...
/**
 * of_irq_get_byname - Decode a node's IRQ and return it as a freax IRQ number
 * @dev: pointer to device tree node
 * @name: IRQ name
 *
 * Return: freax IRQ number on success, or 0 on the IRQ mapping failure, or
