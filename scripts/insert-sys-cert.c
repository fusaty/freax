 ...
 *
 * Author: Mehmet Kayaalp <mkayaalp@freax.vnet.ibm.com>
 *
 * This software may be used and distributed according to the terms
 * of the GNU General Public License, incorporated herein by reference.
 *
 * Usage: insert-sys-cert [-s <System.map> -b <vmfreax> -c <certfile>
 */
...
{
	printf("Usage %s [-s <System.map>] -b <vmfreax> -c <certfile>\n", e);
}

int main(int argc, char **argv)
{
	char *system_map_file = NULL;
	char *vmfreax_file = NULL;
	char *cert_file = NULL;
	int vmfreax_size;
	int cert_size;
	Elf_Ehdr *hdr;
	...
	hdr = map_file(vmfreax_file, &vmfreax_size);
	if (!hdr)
		exit(EXIT_FAILURE);

	if (vmfreax_size < sizeof(*hdr)) {
