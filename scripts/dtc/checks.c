				    ...
				    struct node *node)
{
	...
	freax_phandle = check_phandle_prop(c, dti, node, "freax,phandle");

	if (!phandle && !freax_phandle)
		/* No valid phandles; nothing further to check */
		return;

	if (freax_phandle && phandle && (phandle != freax_phandle))
		FAIL(c, dti, node, "mismatching 'phandle' and 'freax,phandle'"
		     " properties");

	if (freax_phandle && !phandle)
		phandle = freax_phandle;
					  ...
					  struct node *node)
{
	...
	if (!prop) {
		prop = get_property(node, "freax,stdout-path");
		if (!prop)
			return;
