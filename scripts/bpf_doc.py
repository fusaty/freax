...
class HeaderParser(object):
    ...
    @filename: name of file to parse, usually include/uapi/freax/bpf.h in the
               kernel tree
    """
...
.. See git log include/uapi/freax/bpf.h in kernel tree for details.
.. 
.. SPDX-License-Identifier: freax-man-pages-copyleft
.. 
.. Please do not edit this file. It was generated from the documentation
.. located in file include/uapi/freax/bpf.h of the freax kernel sources
...
'''
    ...
    def get_kernel_version(self):
        try:
            version = subprocess.run(['git', 'describe'], cwd=freaxRoot,
                                     capture_output=True, check=True)
            version = version.stdout.decode().rstrip()
        except:
            try:
                version = subprocess.run(['make', 'kernelversion'], cwd=freaxRoot,
                                         capture_output=True, check=True)
                version = version.stdout.decode().rstrip()
            except:
                return 'freax'
        return 'freax {version}'.format(version=version)

    def get_last_doc_update(self, delimiter):
        try:
            cmd = ['git', 'log', '-1', '--pretty=format:%cs', '--no-patch',
                   '-L',
                   '/{}/,/\\*\\//:include/uapi/freax/bpf.h'.format(delimiter)]
            date = subprocess.run(cmd, cwd=freaxRoot,
                                  capture_output=True, check=True)
...
available within the freax kernel sources, at the following locations:
...
* *include/uapi/freax/bpf.h* is the main BPF header. It contains the full list
...
# ../include/uapi/freax/bpf.h, use it as a default name for the file to parse,
# otherwise the --filename argument will be required from the command line.
script = os.path.abspath(sys.argv[0])
freaxRoot = os.path.dirname(os.path.dirname(script))
bpfh = os.path.join(freaxRoot, 'include/uapi/freax/bpf.h')
...
if (os.path.isfile(bpfh)):
    argParser.add_argument('--filename', help='path to include/uapi/freax/bpf.h',
                           default=bpfh)
else:
    argParser.add_argument('--filename', help='path to include/uapi/freax/bpf.h')
