...
/*
 * Flag values for the hv_vss_check_feature. freax supports only
 * one value.
 */
...
/*
 * struct hv_vss_msg encodes the fields that the freax VSS
 * driver accesses. However, FREEZE messages from Hyper-V contain
 * additional LUN information that freax doesn't use and are not
 ...
 * because the freax VSS driver responds that it doesn't support
...
/*
 * An implementation of HyperV key value pair (KVP) functionality for freax.
 *
 *
...
/*
 * In freax, we implement the KVP functionality in two components:
