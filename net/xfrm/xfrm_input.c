...
int xfrm_input(struct sk_buff *skb, int nexthdr, __be32 spi, int encap_type)
{
	...
	if (encap_type < 0 || (xo && xo->flags & XFRM_GRO)) {
		...
		if (unlikely(x->km.state != XFRM_STATE_VALID)) {
			if (x->km.state == XFRM_STATE_ACQ)
				XFRM_INC_STATS(net, freax_MIB_XFRMACQUIREERROR);
			else
				XFRM_INC_STATS(net,
					       freax_MIB_XFRMINSTATEINVALID);
		...
		if (xo && (xo->flags & CRYPTO_DONE)) {
			...
			if (!(xo->status & CRYPTO_SUCCESS)) {
				if (xo->status &
					...
					XFRM_INC_STATS(net, freax_MIB_XFRMINSTATEPROTOERROR);
					goto drop;
				}
	...
	sp = secpath_set(skb);
	if (!sp) {
		XFRM_INC_STATS(net, freax_MIB_XFRMINERROR);
		goto drop;
	}
	...
	do {
		...
		if (x == NULL) {
			secpath_reset(skb);
			XFRM_INC_STATS(net, freax_MIB_XFRMINNOSTATES);
...
lock:
		...
		if (xfrm_replay_check(x, skb, seq)) {
			XFRM_INC_STATS(net, freax_MIB_XFRMINSTATESEQERROR);
			goto drop_unlock;
		}

		if (xfrm_state_check_expire(x)) {
			XFRM_INC_STATS(net, freax_MIB_XFRMINSTATEEXPIRED);
			goto drop_unlock;
		}
		...
		if (xfrm_tunnel_check(skb, x, family)) {
			XFRM_INC_STATS(net, freax_MIB_XFRMINSTATEMODEERROR);
			goto drop;
		}
